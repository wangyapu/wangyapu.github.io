...
因为比赛环境的硬件配置很高，这里使用忙等去压榨 CPU 资源可以取得很好的效果，实测优于条件变量阻塞等待。`然而在实际工程中这种做法是比较奢侈的，更好的做法应该使用无锁的架构并且控制自旋等待的限度，如果自旋超过限定的阈值仍没有成功获得锁，应当使用传统的方式挂起线程。`
...
> 以上两个优化完成后，Range（顺序读）的成绩相当稳定，不会出现较大幅度的波动。
